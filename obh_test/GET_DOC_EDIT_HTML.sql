--------------------------------------------------------
--  DDL for Function GET_DOC_EDIT_HTML
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "GET_DOC_EDIT_HTML" (
    p_treatm_id NUMBER
  , p_lista_id NUMBER
...
v_retval := v_retval || '<th style="text-align:center; overflow:hidden; width:160px;"> Előirányzat neve</td>';

for oszlop in oszlopok
loop
     v_retval := v_retval || '<th style="overflow:hidden; width:125px;">' || oszlop.description || '</th>';
  ...
  LOOP
   ...
   v_retval := v_retval || '</td>'; 
      ...
      loop     
        ...
        v_cell_value := LOOKUP_TR_XL_VALUE(p_treatm_id,p_lista_id,sor.param_type_id,oszlop.order_no,v_tr_xl_id);
