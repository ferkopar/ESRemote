...
BEGIN
  begin
   select language_id into v_lang from dm_user 
    where user_name=NVL(V('APP_USER'),USER);
  exception when others then v_lang:=-1;  
  end;
  RETURN V_LANG;
END v_LANG_F;
...
BEGIN
  ...
  UPDATE vt_DM_TRANSLATE SET names = P_NAME WHERE rowid=ri ;
EXCEPTION
WHEN no_data_found THEN
  INSERT
  INTO vt_DM_TRANSLATE
    (
      NAMES,
      LANGUAGE_ID,
      TBL_NAME,
...
IS
BEGIN
  DELETE vt_DM_TRANSLATE WHERE TBL_NAME=p_TBL_NAME AND TBL_PK_ID=p_TBL_PK_ID;
END DM_TRANSLATE_DEL;
...
BEGIN
  ...
  if p_tab='SUBJECTIV' and v_ret='I' then
   ...
   BEGIN
    SELECT 'Y'
    INTO v_ret
    FROM dm_regulation r,log_table l
...
BEGIN
  SELECT LANGUAGE_NAME INTO v_ret FROM DM_LANGUAGE WHERE LANGUAGE_ID=v_LANG_F;
  RETURN v_ret;
END LANGUAGE_NAME_F;
...
IS
BEGIN
 update dm_user set language_id=p_lang   where user_name=NVL(V('APP_USER'),USER);
END v_LANG_set;
PROCEDURE LANGUAGE_CHANGE(
    p_lang VARCHAR2)
IS
  v_lang_code DM_LANGUAGE.LANGUAGE_CODE%TYPE;
  CURSOR cur
  IS
    SELECT t.apex_variable_page page,SUBSTR(t.apex_variable,2,LENGTH(t.apex_variable)-2) apex_variable ,
      names DISPLAY_NAME
...
null;
  ...
  IF V('APP_ID')!=1111  and 1=2 THEN    
    SELECT LANGUAGE_CODE INTO v_lang_code FROM DM_LANGUAGE WHERE LANGUAGE_ID=(p_lang);
...
BEGIN
  ...
  ELSE
    --vissza:= 1;
    begin
    select language_id into vissza from dm_user where user_name=NVL(V('APP_USER'),USER);
    exception when others then null;
    end;
  END IF;
 return vissza;  
...
BEGIN
  IF p_epi_id IS NOT NULL THEN
    BEGIN
      SELECT EPI_TYPE_ID INTO v_EPI_TYPE_ID FROM epi WHERE epi_id=p_epi_id;
      --vissza:=base_pkg.SUPER_TYP_F(v_EPI_TYPE_ID);
    EXCEPTION
    WHEN OTHERS THEN
      vissza:=p_epi_id||'-error';
    END;
  END IF;
  RETURN vissza;
END EPI_TYPE_F;
...
BEGIN
 begin
  select type_value1 into v_ret from dm_regulation r
   where r.regul_type_id=v_regul_type_id and p_tab=r.type_table1;
 exception when others then null;   
 end;
  RETURN v_ret;
...
BEGIN
    ...
    SELECT * INTO v_rec FROM SUBJECT WHERE subj_id=p_subj_id;
...
IS
BEGIN
  UPDATE DM_USER SET SUBJ_ID=p_subj_new WHERE SUBJ_ID=p_subj_old;
  UPDATE INVENT SET SUBJ_ID=p_subj_new WHERE SUBJ_ID=p_subj_old;
  UPDATE SUBJ_PARAM SET SUBJ_ID=p_subj_new WHERE SUBJ_ID=p_subj_old;
  UPDATE GIS_SUBJ_EPI_REL SET SUBJ_ID=p_subj_new WHERE SUBJ_ID=p_subj_old;
  UPDATE SUBJ_REL SET SUBJ_ID1=p_subj_new WHERE SUBJ_ID1=p_subj_old;
  UPDATE SUBJ_REL SET SUBJ_ID2=p_subj_new WHERE SUBJ_ID2=p_subj_old;
    DELETE SUBJECT WHERE SUBJ_ID=p_subj_old;
END SUBJECTIV_MOVE_P;
FUNCTION EPI_type_F(
    p_epi_id EPI.EPI_ID%type)
  RETURN EPI.EPI_TYPE_ID%type
IS
  v_ret EPI.EPI_TYPE_ID%type;
BEGIN
  SELECT epi_type_id INTO v_ret FROM epi WHERE epi_id=p_epi_id;
  RETURN v_ret;
END EPI_type_F;
FUNCTION gis_f(
...
PROCEDURE TREATM_REL_pkg ( 
 ...
 CURSOR CUR_PAR IS SELECT * FROM TREATM_PARAM WHERE TREATM_ID=P_TREATM_ID1 ORDER BY ORDER_NO,TREATM_PARAM_ID;
 p_TREATM_PARAM_ID TREATM_PARAM.TREATM_PARAM_ID%type;
BEGIN
 FOR REC IN CUR_PAR LOOP
  if p_mode='I' then
  BEGIN
   select TREATM_PARAM_ID into p_TREATM_PARAM_ID from TREATM_PARAM where TREATM_ID=P_TREATM_ID2 and PARAM_TYPE_ID=rec.PARAM_TYPE_ID;
...
BEGIN
 IF P_MODE='S' THEN
  ...
  BEGIN
   SELECT 'I' INTO VISSZA
    FROM SUBJ_PARAM
    WHERE SUBJ_ID=P_SUBJ_ID AND PARAM_TYPE_ID=P_TYPE_ID
      and rownum<2;
  exception when others then null;
  end; 
 END IF;
...
procedure image_display( p_mm_id mm_files.mm_id%type) is
 l_mime        VARCHAR2 (255);
   ...
   lob_loc       BLOB;
BEGIN
        SELECT MAIN_TYPE, MM_BLOB, DBMS_LOB.getlength (MM_BLOB)
