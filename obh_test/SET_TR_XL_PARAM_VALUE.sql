...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "SET_TR_XL_PARAM_VALUE" 
  (
    p_tr_xl_id NUMBER,
    p_value VARCHAR2
  )
...
BEGIN
  SELECT SEQ_BASE.NEXTVAL INTO v_new_tr_xl_id from DUAL;
  SELECT * INTO v_tr_xl_row FROM tr_xl WHERE tr_xl_id = p_tr_xl_id;
  SELECT * INTO v_col_dm_xl_row FROM dm_xl WHERE dm_xl_id = v_tr_xl_row.COL_DM_XL_ID;
  UPDATE tr_xl tx SET status_id = 919 WHERE tx.tr_xl_id = p_tr_xl_id;
  ...
  UPDATE TR_XL set TR_XL_ID=v_new_tr_xl_id where TR_XL_ID = v_tr_xl_row.TR_XL_ID;
  INSERT INTO tr_xl (tr_xl_id,list_type_id,param_date,param_type_id,param_value,
    param_unit,subj_id,column_no,data_inco_id,status_id,order_no,
    param_date_type_id,param_adate_type_id,param_fdate,param_tdate,COLUMN_DATA_TYPE_ID,PARAM_VALUE_CHR,COL_DM_XL_ID,ROW_DM_XL_ID)
    VALUES 
    (v_tr_xl_row.tr_xl_id,v_tr_xl_row.list_type_id,v_tr_xl_row.param_date,v_tr_xl_row.param_type_id,v_cell_value,
    v_tr_xl_row.param_unit,v_tr_xl_row.subj_id,v_tr_xl_row.column_no,v_tr_xl_row.data_inco_id,
    916,v_tr_xl_row.order_no,
    v_tr_xl_row.param_date_type_id,v_tr_xl_row.param_adate_type_id,v_tr_xl_row.param_fdate,v_tr_xl_row.param_tdate
    ,v_tr_xl_row.COLUMN_DATA_TYPE_ID,p_value,v_tr_xl_row.COL_DM_XL_ID,v_tr_xl_row.ROW_DM_XL_ID);
/*
    for x in 
      (select 
