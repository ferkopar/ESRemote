...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "TR_XL_INSERT_OR_UPDATE" (
    p_value NUMBER,
    p_doc_id NUMBER,
    p_list_id NUMBER,
...
BEGIN
  select * into v_dm_xl_col FROM DM_XL WHERE REL_TYPE_ID =42 AND LIST_TYPE_ID = p_list_id AND ORDER_NO =to_char( p_col_order_no);
  begin
  ...
  AND STATUS_ID != 919;
  UPDATE TR_XL SET PARAM_VALUE = p_value,
                   PARAM_VALUE_CHR = TO_CHAR(p_value)
  WHERE TR_XL_ID = v_tr_xl_row.TR_XL_ID;
  EXCEPTION 
  WHEN NO_DATA_FOUND THEN 
      INSERT INTO TR_XL ( LIST_TYPE_ID, DATA_INCO_ID, COLUMN_NO, ORDER_NO, PARAM_VALUE, PARAM_VALUE_CHR, PARAM_TYPE_ID, COLUMN_DATA_TYPE_ID , col_dm_xl_id, row_dm_xl_id) 
  ...
  WHEN OTHERS THEN
      htp.p('hiba:TR_XL_INSERT_OR_UPDATE');
  end;
